...
metadata {
	...
	definition (name: "derek - Iris Smart Plug", namespace: "derek - blebson", author: "derek - SmartThings") {
		capability "Actuator"
		capability "Switch"
	...
	tiles(scale: 2) {
		multiAttributeTile(name:"switch", type: "lighting", width: 4, height: 2, canChangeIcon: true){
			tileAttribute ("device.switch", key: "PRIMARY_CONTROL") {
				attributeState "on", label: '${name}', icon: "st.switches.switch.on", backgroundColor: "#79b821"
        ...
        valueTile("energyCost", "device.energyCost", width: 5, height: 1, decoration: "flat") {
			state "default", label:'Total Cost: ${currentValue}'
        	}
        standardTile("cost", "device.switch", inactiveLabel: false, decoration: "flat", width: 1, height: 1) {
			state "default", label:"\$0.12"
		}   
		...
		main "switch"
		details(["switch","energyCost","cost","energyDisplay","resetUsage","power","elapsedTimeDisplay","refresh"])
	}
}
    ...
    */
	...
	if (finalResult) {
		...
		if (finalResult.type == "update") {
			...
			sendEvent(name: "power", value: powerValue, isStateChange: true, displayed: true) // note: stateChange = true added so the energy calculation can work properly
			/*
...
def calculateAndShowEnergy()
{
    ...
    def energyCost = energyValue * 0.15
    	log.debug "Energy C is at $energyCost"
    ...
    sendEvent(name: "energyCost", value:  String.format("\$%6.3f",energyCost), displayed: false)
